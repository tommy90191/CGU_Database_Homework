                    ...
                    if(row[-3].find("推")+1):
                       # print(row[-3])
                        ...
                        if(int(row[-3].strip().split(' ')[-1].split("/")[0])>=int(start_month) and int(row[-3].strip().split(' ')[-1].split("/")[0])<=int(end_month)):
                            print("推文", row[2])
                    elif (row[-3].find("推")-1) and len(row[-3].split("."))>1:
                       # print(row[-3].strip().split(' ')[1])
                        if (int(row[-3].strip().split(' ')[1].split("/")[1]) >= int(start_month) and int(
                            row[-3].strip().split(' ')[1].split("/")[1]) <= int(end_month)):
                            print("推文", row[2])
                    else:
                        #print(row[-3].strip().split(' ')[0].split("/")[1]+"/"+row[-3].strip().split(' ')[0].split("/")[2])
                       # print(row[-3].strip().split(' ')[0])
                    ...
                    for i in month_list[int(start_month)-1:int(end_month)-1] :
                        if(len(row[-1].split(' '))==6):
                            ...
                            if (i == row[-1].split(' ')[1]):
                                print("發文 標題:"+row[1]+" 內容:"+row[-2][0:20]+"...")
    if(select=="2"):
        kind=input('想要搜尋哪個板的資料(joke,HatePolitics,MobileComm,movie):')
        print("推文最多(前三名)")
        with open('message.csv', newline='',encoding="utf-8") as csvfile:
            user_list=[]
            rows = csv.reader(csvfile)

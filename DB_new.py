...
while(1):
    ...
    if(select=="3"):
        ...
        row=[]
        print("推文")
        with open('message_new.csv', newline='',encoding="utf-8") as csvfile:
        ...
        with open('title_new.csv', newline='', encoding="utf-8") as csvfile:
            rows = csv.reader(csvfile)
            #print(month_list[int(start_month) - 1],month_list[int(end_month) - 1])
            for row in rows:
                if (row[2].find(id_num)):
                    # print(row[0],row[2],row[-2])
    ...
    if(select=="1"):
        ...
        with open('message_new.csv', newline='', encoding="utf-8") as csvfile:
            ...
            for row in rows:
                ...
                if (row[1] =="joke"):
                    m_joke=m_joke+1
                if (row[1] =="PC_Shopping"):
                    m_PC_Shopping=m_PC_Shopping+1

        plt.subplot(2, 2, 1)
        plt.plot([0, 1], [0, 1])
        size = [m_joke,  m_HatePolitics, m_MobileComm, m_movie,m_PC_Shopping]
        labels = ['joke', 'HatePolitics', 'MobileComm', 'movie','PC_Shopping']
       ...
       # print("發文最多(前三名)")
        with open('title_new.csv', newline='', encoding="utf-8") as csvfile:
            ...
            for row in rows:
                ...
                if (row[-3] =="joke"):
                    t_joke=t_joke+1
                if (row[-3] =="PC_Shopping"):
                    t_PC_Shopping=t_PC_Shopping+1

        plt.subplot(2, 2, 2)
        size = [t_joke,  t_HatePolitics, t_MobileComm, t_movie,t_PC_Shopping]
        labels = ['joke', 'HatePolitics', 'MobileComm', 'movie','PC_Shopping']
        plt.pie(size, labels=labels, autopct='%1.1f%%')
        plt.axis('equal')
        ...
        plt.subplot(2, 2, 3)
        size = [m_joke + t_joke, m_HatePolitics + t_HatePolitics, m_MobileComm + t_MobileComm, m_movie + t_movie, m_PC_Shopping + t_PC_Shopping]
        labels = ['joke', 'HatePolitics', 'MobileComm', 'movie','PC_Shopping']
        plt.pie(size, labels=labels, autopct='%1.1f%%')
        plt.axis('equal')
